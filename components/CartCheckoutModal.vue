    ...
    <div class="modal-content" @click.stop>
      ...
      <div v-if="!orderSent && !showError" class="modal-body">
        <div class="cart-summary">
          <div v-for="item in items" :key="item.cassetteId" class="summary-item">
            <span class="summary-item-title">{{ item.artist }} — {{ item.title }}</span>
          ...
          <div class="summary-totals">
            ...
            <div v-if="discount > 0" class="summary-row discount-row">
              <span>Скидка (3 кассеты = 3000 ₽)</span>
              <span>-{{ discount }} ₽</span>
            </div>
        ...
        <form @submit.prevent="submitOrder">
          ...
          <div class="form-group">
            <label for="checkout-contact" class="form-label">Контакт *</label>
            <input
              type="text"
              id="checkout-contact"
          ...
          <div class="form-group">
            <label for="checkout-telegram" class="form-label">Telegram</label>
            <input
              type="text"
          ...
          <div class="form-group">
            <label for="checkout-message" class="form-label">Сообщение *</label>
            <textarea
              id="checkout-message"
...
export default {
  ...
  setup() {
    ...
    return { items, subtotal, total, discount, clearCart }
  },
  data() {
    return {
      formData: {
  ...
  watch: {
    show(newVal) {
      if (newVal) {
        this.formData.message = this.buildCartSummary()
      }
    }
  },
  methods: {
    buildCartSummary() {
      const lines = this.items.map(item => `${item.artist} — ${item.title} (${item.price} ₽)`)
      let summary = `Хочу заказать кассеты:\n${lines.join('\n')}`
      summary += `\n\nИтого: ${this.total} ₽`
      if (this.discount > 0) {
        summary += ` (скидка ${this.discount} ₽)`
      }
      return summary
    },
    ...
    closeModal() {
      this.$emit('close')
      setTimeout(() => {
        this.formData = { name: '', contact: '', telegram: '', message: '' }
    ...
    async submitOrder() {
      ...
      try {
        let message = `Заказ из корзины (${this.items.length} кассет)\n\n`
        message += this.items.map(item => `- ${item.artist} — ${item.title}`).join('\n')
        message += `\n\nСумма: ${this.subtotal} ₽`
        if (this.discount > 0) {
